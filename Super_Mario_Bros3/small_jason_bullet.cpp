...
small_jason_bullet::small_jason_bullet()
{
	SetState(CWAVE_BULLET_STATE_FLYING);
	nx = 0;
}

void small_jason_bullet::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x;
	top = y;
...
void small_jason_bullet::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	if (isUsed == false)
	{
		...
		MINI_JASON* JASON = ((CPlayScene*)CGame::GetInstance()->GetCurrentScene())->GetPlayer3();
...
void small_jason_bullet::CalcPotentialCollisions(
...
void small_jason_bullet::Render()
{
	if (state == CWAVE_BULLET_STATE_DIE)
		return;
	int ani = 0;
...
void small_jason_bullet::SetState(int state)
{
